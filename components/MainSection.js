    ...
    return (
      <section className="main">
        ...
        <ul className="todo-list">
          {visibleIds.map(id => <TodoItem key={id} id={id} />)}
        </ul>
        {this.renderFooter()}
      </section>
    );
...
const getCompletedCount = createSelector(
  [state => state.todos.listedIds, state => state.todos.isCompletedById],
...
export default connect(mapStateToProps, { completeAll, clearCompleted })(
  MainSection
);
